...
class GapoktanForm(QWidget):
    PK = "id_gapoktan"
    def __init__(self, parent=None):
        super().__init__(parent)
        self.w = _load_ui("GapoktanForm.ui"); self.w.setParent(self)
        self.w.setWindowTitle("Gapoktan (Sederhana)")

        # Opsional: isi ComboBox lokasi jika objectName = id_lokasi
        try:
            cb = self.w.findChild(QComboBox, "id_lokasi")
            if cb is not None:
                cb.clear()
                for r in crud.DB.lokasi_list():
                    cb.addItem(f"{r['id_lokasi']} - {r['nama']}", r["id_lokasi"])
        except Exception:
            pass
    ...
    def on_save(self):
        ...
        try:
            if not pk:
                new_id = crud.gapoktan_insert(data)
                ...
                rows = crud.gapoktan_update(pk, data)
