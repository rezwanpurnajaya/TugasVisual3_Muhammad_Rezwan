...
class PetaniForm(QWidget):
    PK = "id_petani"
    def __init__(self, parent=None):
        super().__init__(parent)
        self.w = _load_ui("PetaniForm.ui"); self.w.setParent(self)
        self.w.setWindowTitle("Petani (Sederhana)")

        # Opsional: isi ComboBox poktan jika objectName = id_poktan
        try:
            cb = self.w.findChild(QComboBox, "id_poktan")
            if cb is not None:
                cb.clear()
                for r in crud.DB.poktan_list():
                    cb.addItem(f"{r['id_poktan']} - {r['nama']}", r["id_poktan"])
        except Exception:
            pass
    ...
    def on_save(self):
        ...
        try:
            if not pk:
                new_id = crud.petani_insert(data)
                ...
                rows = crud.petani_update(pk, data)
