...
class PoktanForm(QWidget):
    PK = "id_poktan"
    def __init__(self, parent=None):
        super().__init__(parent)
        self.w = _load_ui("PoktanForm.ui"); self.w.setParent(self)
        self.w.setWindowTitle("Poktan (Sederhana)")

        # Opsional: isi ComboBox gapoktan jika objectName = id_gapoktan
        try:
            cb = self.w.findChild(QComboBox, "id_gapoktan")
            if cb is not None:
                cb.clear()
                for r in crud.DB.gapoktan_list():
                    cb.addItem(f"{r['id_gapoktan']} - {r['nama']}", r["id_gapoktan"])
        except Exception:
            pass
    ...
    def on_save(self):
        ...
        try:
            if not pk:
                new_id = crud.poktan_insert(data)
                ...
                rows = crud.poktan_update(pk, data)
